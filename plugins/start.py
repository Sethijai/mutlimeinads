    ...
    if len(text) > 7:
        ...
        try:
            ...
            if string.startswith("HACKHEIST-"):
                ...
                if len(parts) >= 4 and parts[0] == "HACKHEIST":
                    try:
                        user_id = int(parts[1])
                        msg_id = int(parts[2]) // 8  # Reverse *8
                        # Handle channel_id, which may include negative sign
                        channel_id_str = "-".join(parts[3:])  # Rejoin remaining parts
                        ...
                        k = await client.send_message(
                            chat_id=message.from_user.id,
                            text=f"<b>‼️ 𝐓𝐡𝐢𝐬 𝐋𝐄𝐂𝐓𝐔𝐑𝐄/𝐏𝐃𝐅 𝐀𝐮𝐭𝐨𝐦𝐚𝐭𝐢𝐜 𝐃𝐞𝐥𝐞𝐭𝐢𝐧𝐠 𝐢𝐧 𝟯 �_D𝗮𝘆𝘀 💀</b>\n\n"
        ...
        try:
            ...
            if len(argument) == 5 and argument[1] == "":
                channel_id = int(f"-{argument[2]}") // 8  # Reverse *8
                f_msg_id = int(argument[3]) // 8  # Reverse *8
                s_msg_id = int(argument[4]) // 8  # Reverse *8
                ...
                channel_id = int(argument[1]) // 8  # Reverse *8
                f_msg_id = int(argument[2]) // 8  # Reverse *8
                s_msg_id = int(argument[3]) // 8  # Reverse *8
        ...
        for msg in messages:
            ...
            base64_string2 = await encode_new(f"HACKHEIST-{user_id}-{msg.id * 8}-{channel_id * 8}")
