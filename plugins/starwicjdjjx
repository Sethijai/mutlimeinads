...
async def start_command(client: Client, message: Message):
    ...
    if len(text) > 7:
            ...
            if string.startswith("HACKHEIST-"):
                ...
                if len(argument) == 4 and argument[0] == "HACKHEIST":
                    try:
                        ...
                        channel_id = int(argument[3])  # No *8 modification
                        ...
                        k = await client.send_message(
                            ...
                            text=f"<b>‼️ 𝐓𝐡𝐢𝐬 𝐋𝐄𝐂𝐓𝐔𝐑𝐄/𝐏𝐃𝐅 𝐀𝐮𝐭𝐨𝐦𝐚𝐭𝐢𝐜 𝐃𝐞𝐥𝐞𝐭𝐢𝐧𝐠 𝐢𝐧 𝟯 𝗗𝗮𝘆𝘀 💀</b>\n\n"
                                 ...
                                 f"<b> <a href=https://yashyasag.github.io/hiddens_officials>🌟 𝗢𝗧𝗛𝗘𝗥 𝗪�_E𝗕𝗦𝗜𝗧𝗘𝗦 🌟</a></b>",
                        )
        ...
        # Batch message handling with new format
        try:
            string = await decode(base64_string)
            print(f"Decoded batch string: {string}")  # Debug log
            argument = string.split("-")
            ...
            if len(argument) == 5 and argument[1] == "":  # Case: get--channel_id-f_msg_id-s_msg_id
                ...
                print(f"Negative channel ID format - channel_id: {channel_id}, f_msg_id: {f_msg_id}, s_msg_id: {s_msg_id}")  # Debug
                ...
                print(f"Positive channel ID format - channel_id: {channel_id}, f_msg_id: {f_msg_id}, s_msg_id: {s_msg_id}")  # Debug
                if f_msg_id <= s_msg_id:
                    ids = list(range(f_msg_id, s_msg_id + 1))
                else:
                    ids = list(range(f_msg_id, s_msg_id - 1, -1))
                ...
                channel_id = int(f"-{argument[2]}")
                f_msg_id = int(argument[3])
                print(f"Single negative ID format - channel_id: {channel_id}, f_msg_id: {f_msg_id}")  # Debug
                ids = [f_msg_id]
            elif len(argument) == 3 and argument[1] != "":  # Case: get-channel_id-f_msg_id
                channel_id = int(argument[1])
                f_msg_id = int(argument[2])
                print(f"Single positive ID format - channel_id: {channel_id}, f_msg_id: {f_msg_id}")  # Debug
                ids = [f_msg_id]
            else:
        ...
        try:
            messages = await get_messages(client, ids, channel_id)  # Match your get_messages signature
        ...
        for msg in messages:
            ...
            base64_string2 = await encode_new(f"HACKHEIST-{user_id}-{msg.id}-{channel_id}")  # No *8 modification
        ...
        k = await client.send_message(
            ...
            text=f"<b>‼️ 𝐓𝐡𝐞𝐬𝐞 𝐋𝐄𝐂𝐓𝐔𝐑𝐄𝐒/𝐏𝐃𝐅𝐬 𝐀𝐮𝐭𝐨𝐦𝐚𝐭𝐢𝐜 𝐃𝐞𝐥𝐞𝐭𝐢𝐧𝐠 𝐢𝐧 𝟰 𝗛𝗼𝘂𝗿𝘀 🔥</b>\n\n"
                 f"<b>𝘚𝘰 𝘍𝘰𝘳 𝘚𝘢𝘷𝘪𝘯𝘨 𝘓𝘦𝘤𝘵𝘶𝘳𝘦/𝘗𝘥𝘧 𝘤𝘭𝘪𝘤𝘬 𝘰𝘯 𝘣𝘦𝘭𝘰𝘸 𝘣𝘶𝘴𝘁𝘁𝘰𝘯(😁 𝗖𝗟𝗜𝗖𝗞 𝗧𝗢 𝗦𝗔𝗩𝗘 📥) 𝘰𝘧 𝘸𝘩𝘪𝘤𝘩 𝘓𝘦𝘤𝘵𝘶𝘳𝘦/𝘗𝘥𝘧 𝘺𝘰𝘶 𝘸𝘢𝘯𝘵 𝘵𝘰 𝘴𝘢𝘷𝘦 𝘣𝘺 𝘵𝘩𝘪𝘴 𝘵𝘩𝘢𝘵 𝘭𝘦𝘤𝘵𝘶𝘳𝘦/𝘗𝘥𝘧 𝘠𝘰𝘶 𝘤𝘢𝘯 𝘚𝘢𝘷𝘦 𝘪𝘯 𝘎𝘢𝘭𝘭𝘦𝘳𝘺 😊</b>\n\n"
                 f"<b>ʙᴜᴛ ᴅᴏɴ'ᴛ ᴡᴏʀʀʏ 😁 ᴀғᴛᴇʀ ᴅᴇʟᴇᴛᴇᴅ ʏᴏᴜ ᴄᴀɴ ᴀɢᴀɪɴ ᴀᴄᴄᴇss ᴛʜʀᴏᴜɢʜ ᴏᴜʀ ᴡᴇʙsɪᴛᴇs 😘</b>\n\n"
